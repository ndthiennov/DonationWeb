...
@Component({
  selector: 'app-campaign-detail-recipient',
  standalone: true,
...
export class CampaignDetailRecipientComponent implements OnInit {
  ...
  ReceivedConfirmation() {
    let receivedConfirm = true;
    if (this.campaign.received == "Received") {
      receivedConfirm = false;
    }
    this.campaignService.UpdateRecivedByRecipient(this.campaign.id, JSON.stringify(receivedConfirm)).subscribe(
      (res: any) => {
        if (receivedConfirm) {
          this.campaign.received = 'Received';
          this.received = true;
        }
        else {
          this.campaign.received = 'Unreceived';
          this.received = false
        }
      },
      (err: any) => {
  ...
  InitRateForm(): void {
    ...
    let recipientId: number = +this.userId;

    this.rateForm = new FormGroup({
      'campaignid': new FormControl(campaignId, Validators.required),
      'donorid': new FormControl(recipientId, Validators.required),
  ...
  Rate() {
    ...
    this.campaignService.UpdateRatedByRecipient(this.campaign.id, this.rateForm.value).subscribe(
      (res: any) => {
        if (res.type === HttpEventType.Response) {
  ...
  ReviewRating() {
    ...
    for (let i = 0; i < 5; i++) {
      allStar[i].classList.replace('bi-star-fill', 'bi-star')
    }
